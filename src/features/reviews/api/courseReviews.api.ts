...
import { listCourseReviews } from "../../../generated/api/anonymous-review/anonymousReviewService";
import type { AnonymousReview } from "../../../generated/api/anonymous-review/models";
import { getAuthHeaders, handleAuthenticatedResponse } from "../../auth";
import { listReviews } from "../../../generated/api/review/reviewService";
import type { Review as AuthenticatedReview } from "../../../generated/api/review/models";
import type {
	CourseRatings,
	Review,
	ReviewSort,
...
function toReview(courseId: string, review: AnonymousReview): Review {
	// TODO(backend): Once anonymous reviews include display metadata, replace this adapter with the generated useListCourseReviews hook.
...
export function authenticatedReviewToReview(
	...
	// TODO(backend): Once authenticated reviews include display metadata, replace this adapter with the generated useListReviews/useGetReview hooks.
	const term = fallbackTerm(courseId, review.id);
	const reactions = fallbackReactionCounts(courseId, review.id);
...
function sortLoadedReviews(reviews: Review[], sort: ReviewSort): Review[] {
	if (sort === "top") {
		...
		// TODO(backend): Once top ordering is in listCourseReviews/listReviews params, switch callers to the generated query hook with server-side sort params.
		return [...reviews].sort(
...
export async function getCourseReviewsPage(
	...
	// TODO(backend): Once review list responses include cursor/total metadata and all card fields, replace this adapter with the generated useListCourseReviews hook.
...
export async function getOwnCourseReview(
	...
	// TODO(backend): Once the backend exposes a viewer-owned review endpoint or full review shape, replace this lookup adapter with the generated hook for that endpoint.
