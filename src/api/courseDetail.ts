...
import { getCourse } from "./course/courseService";
import type { GetCourseResponseContent } from "./course/models";
import type {
	CourseRatings,
	Curso,
	CursoListItem,
...
function fallbackSemesters(courseId: string): RamoSummary[] {
	...
	return withCourseSeed(courseId, 100, () => {
		...
		return terms.length > 0
			? terms
			: [{ year: 2026, semester: 1, sections: [{ id: "2026-1-1", professors: [] }] }];
	});
}
...
function fallbackReviewCount(course: GetCourseResponseContent): number {
	...
	return values.length > 0
		? withCourseSeed(course.id, 500, () => faker.number.int({ min: 4, max: 48 }))
		: 0;
}
...
export function getRelatedCourses(courseId: string, count: number): CursoListItem[] {
